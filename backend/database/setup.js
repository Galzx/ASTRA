...
db.serialize(() => {
  ...
  db.get(`SELECT COUNT(*) AS count FROM knowledge`, [], (err, row) => {
    ...
    if (err) {
      console.error("Failed to check knowledge table:", err);
      return;
    }

    if (row.count > 0) {
      console.log("Knowledge table already seeded, skipping insert.");
      return;
    }
    ...
    `, (err) => {
      if (err) {
        console.error("Failed to seed knowledge table:", err);
      } else {
        console.log("Knowledge table seeded.");
      }
    });
